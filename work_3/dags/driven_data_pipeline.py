...
def _write_to_csv() -> None:
    ...
    # *** CAMBIO IMPORTANTE AQUÍ: mode="w" para sobrescribir el archivo y evitar encabezados duplicados ***
    with open("/opt/airflow/data/raw_data.csv", mode="w", encoding="utf-8", newline="") as file:
        writer = csv.writer(file)
        writer.writerow(headers) # Se escribe el encabezado solo una vez por ejecución
...
def _update_datetime() -> None:
    ...
    if str(date.today()) != "2024-09-23":
        ...
        df = pl.read_csv(
            "/opt/airflow/data/raw_data.csv",
            # Es buena idea añadir los dtypes aquí también por si acaso
            dtypes={
                "personal_number": pl.Utf8,
                "session_duration": pl.Utf8,
                "download_speed": pl.Utf8,
                "upload_speed": pl.Utf8,
                "consumed_traffic": pl.Utf8,
                "unique_id": pl.Utf8 # unique_id también será Utf8
            }
        )
...
# *** NUEVA TAREA AÑADIDA: Truncar la tabla antes de cargar datos ***
truncate_raw_table_task = SQLExecuteQueryOperator(
    task_id='truncate_raw_table',
    conn_id='postgres_default',
    sql='TRUNCATE TABLE driven_raw.raw_batch_data;',
    dag=dag,
)
...
load_raw_data_task = SQLExecuteQueryOperator(
    ...
    CSV HEADER;
    """,
    dag=dag # Asegúrate de que dag=dag esté aquí para este operador también
)
...
# *** CAMBIO EN DEPENDENCIAS: Añadida truncate_raw_table_task ***
create_raw_table_task >> truncate_raw_table_task >> load_raw_data_task >> run_dbt_staging_task
