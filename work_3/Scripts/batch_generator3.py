...
def create_data(locale: str) -> Faker:
    ...
    """
    # Log the action.
    # Modificación aquí: el mensaje de log para reflejar el locale, no solo el país
    logging.info(f"Created synthetic data for locale: {locale}.")
    return Faker(locale)
...
def generate_record(fake: Faker) -> list:
    ...
    # Modificación aquí: fake.address() ya genera la dirección en el idioma del locale
...
def write_to_csv(file_path: str, rows: int) -> None:
    ...
    """
    # *** MODIFICACIÓN CLAVE AQUÍ: Cambiar "ro_RO" a "es_MX" ***
    fake = create_data("es_MX") # Ahora generará datos en español de México
...
if __name__ == "__main__":
    ...
    # *** MODIFICACIÓN AQUÍ: Ajustar la ruta de salida para que sea compatible con Chapter 3 / work_3 / data / ***
    # La práctica sugiere "Chapter 3/work_3/data/"
    # Asegúrate de que esta ruta exista o cámbiala según tu estructura real.
    output_file = f"Chapter 3/work_3/data/batch_{date.today()}.csv"

    # Define number of records: first run - 10_372; next runs random number.
    # Aquí puedes ajustar los números si quieres generar más o menos datos para tus pruebas.
    # La fecha "2024-09-14" es una fecha específica, si la cambias, cambiará el comportamiento del generador.
    # Para tu práctica, si quieres una generación consistente, puedes simplemente usar un número fijo.
    if str(date.today()) == "2024-09-14": # Esta fecha es del ejemplo, no necesariamente la de hoy.
        records = random.randint(100_372, 100_372) # Genera 100372 registros
        run_type = "first"
    else:
        records = random.randint(100, 1101) # Genera un número aleatorio entre 100 y 1101
        run_type = "next"
